...
_Note: If the MMRQ is set while the IDLE character has already elapsed, mute mode is not entered (RWU is not set)._

_If the LPUART is activated while the line is IDLE, the idle state is detected after the duration of one IDLE frame (not only after the reception of one character frame)._
...
_Note: In 7-bit and 9-bit data modes, address detection is done on 6-bit and 8-bit addresses (ADD[5:0] and ADD[7:0]) respectively._
...
_Note: When FIFO management is enabled, when MMRQ bit is set while the receiver is sampling the last bit of a data, this data may be received before effectively entering in mute mode._

An example of mute mode behavior using address mark detection is given in _Figure 360_.
