...
_Note: The UE DMA request signal lptim_ue_dma is reset in the following conditions:_

_-_ _if the corresponding DMA request is disabled (clear UEDE bit in the LPTIM_DIER register)_

_-_ _or if the LPTIM is disabled (clear the ENABLE bit in the LPTIM_CR register)_

_-_ _or if the channel x is disabled (clear CCxE bit) and all the other channels are already disabled_
...
_Note: All DMA requests must be disabled (reset UEDE and CCxDE bits) before entering Sleep, Stop and Standby modes._
