...
_Note: The UE DMA request signal lptim_ue \_dma is reset in the following conditions:_

_- if the corresponding DMA request is disabled (clear UEDE bit in the LPTIM_DIER register)_

_- or if the LPTIM is disabled (clear the ENABLE bit in the LPTIM_CR register)_

_- or if the channel x is disabled (clear CCxE bit) and all the other channels are already disabled_
