...
_Note: If OCx is selected on TIM_3 \*as the trigger output (MMS=1xx), its rising edge is used to clock the counter of TIM_2 _.\*

#### Using one timer to enable another timer

In this example, we control the enable of TIM2 with the output compare 1 of Timer 3. Refer
to _Figure 224_ for connections. TIM2 counts on the divided internal clock only when OC1REF
...
_Note: The counter_ 2 _clock is not synchronized with counter 1, this mode only affects the TIM_2
\_counter enable signal._

**Figure 226. Gating TIM2 with OC1REF of TIM3**

In the example in _Figure 226_, the TIM2 counter and prescaler are not initialized before being
