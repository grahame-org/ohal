...
This bit-field indicates the division ratio between the timer clock (CK*INT) frequency and the
...
\_Note:* \_t **DTS**= 1/f **DTS**, t **CK_INT**= 1/f **CK_INT**.\_
...
interrupt flags of channels configured in output (CCxS=00 in TIMx*CCMRx register) are
...
\_Note:* \_Switch from edge-aligned mode to center-aligned mode as long as the counter is enabled (CEN=1) is not allowed\_
