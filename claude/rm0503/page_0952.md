...
mode (TAMPFLT ≠ 00). Refer to _Section : Level detection with filtering on tamper inputs_
\*(passive mode)\_.
...
_Note: TSF is set up to 3 ck_apre cycles after TAMPxF flags. TSF is not set if RTCCLK is stopped_
_(it is set when RTCCLK restarts)._
