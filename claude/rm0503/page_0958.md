...
_The tamper 3 interrupt must not be enabled when TAMP3MSK is set._
...
hardware. The backup registers and device secrets(1) are not erased.
_The tamper 2 interrupt must not be enabled when TAMP2MSK is set._
...
hardware. The backup registers and device secrets(1) are not erased.
_The tamper 1 interrupt must not be enabled when TAMP1MSK is set._
...
1. The effects of tamper detection in confirmed mode is described with tamp_confirmed signal in _Table 181: TAMP interconnection_.
2. The effects of tamper detection in potential mode is described with tamp_potential signal in _Table 181: TAMP interconnection_.
