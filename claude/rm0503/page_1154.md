...
\_Note:* _The IDLE bit is not set again until the RXFNE bit has been set (that is, a new idle line_
_occurs)._
...
\_Note:* _When this bit is set, the LPUART_RDR register content is not lost but the shift register_
...
1: Noise is detected
\_Note:* _This bit does not generate an interrupt as it appears at the same time as the RXFNE bit_
...
_This error is associated with the character in the LPUART_RDR._
