...
_Note: Due to USB data rate and packet memory interface requirements, the APB clock must have a minimum frequency of 12 MHz to avoid data overrun/underrun problems._
...
byte boundary. Buffer descriptor table entries are described in _Section 37.7: USBSRAM registers_. If an endpoint is unidirectional and it is neither an isochronous nor a double-
...
special handling of packet buffers (Refer to _Section 37.5.5: Isochronous transfers in Device mode_ and _Section 37.5.3: Double-buffered endpoints and usage in Device mode_
...
For Host mode different sections explain the buffer usage model, notably _Section 37.5.6: Isochronous transfers in Host mode_ and _Section 37.5.4: Double buffered channels: usage in Host mode_.
