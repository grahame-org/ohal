...
a minimum frequency of 12 MHz to avoid data overrun/underrun problems.
...
byte boundary. Buffer descriptor table entries are described in Section 37.7: USBSRAM
registers. If an endpoint is unidirectional and it is neither an isochronous nor a double-
...
special handling of packet buffers (Refer to Section 37.5.5: Isochronous transfers in Device
mode and Section 37.5.3: Double-buffered endpoints and usage in Device mode
respectively). The relationship between buffer description table entries and packet buffer
areas is depicted in Figure 385.

For Host mode different sections explain the buffer usage model, notably Section 37.5.6:
Isochronous transfers in Host mode and Section 37.5.4: Double buffered channels: usage in
Host mode.
