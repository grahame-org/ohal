...
5. Write the AES*DINR register four times to input the cipher text (MSB first), as shown in
   \_Figure 108*.
6. Wait until the CCF flag is set in the AES_SR register.
7. Read the AES*DOUTR register four times to get the plain text (MSB first), as shown in
   \_Figure 108*. Then clear the CCF flag by setting the CCF bit of the AES_CR register.
8. Repeat steps _5_-_6_-_7_ to process all the blocks encrypted with the same key.

**Figure 108. ECB/CBC decryption (Mode 3)**

| WR CT0 | Wait until flag CCF = 1 | RD PT3 |
| ------ | ----------------------- | ------ |
