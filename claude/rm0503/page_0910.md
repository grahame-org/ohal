...
In addition, it is possible to output RTC*OUT2 on PB2 pin thanks to OUT2EN bit. The
different functions are mapped on RTC_OUT1 or on RTC_OUT2 depending on OSEL, COE
and OUT2EN configuration, as shown in \_Table 174: RTC_OUT mapping*.
...
prescalers (see _Figure 300: RTC block diagram_):
...
_Note: When both prescalers are used, it is recommended to configure the asynchronous prescaler to a high value to minimize consumption._
