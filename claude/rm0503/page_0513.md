...
_Note:_
_AES is tolerant of delays between consecutive read or write operations, which allows, for_
...
2. Configure the DMA controller so as to transfer the data to process from the memory to
   ...
   last four-word block if padded with zeros. The sequence described in _Data append_
   _through polling_ must be used instead for this last block, because NPBLB bits must be
...
_Note:_
_The CCF flag has no use with this method, because the reading of the AES_DOUTR_
