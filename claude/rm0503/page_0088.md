...
For flash memory erase operation,the flag is set when setting STRT bit of _FLASH control_
_register (FLASH_CR)_, and it is cleared when the operation completes or ends with an error.
_Note:_ _The flag is also set with the first byte of an access to the locked flash memory. In this_
_case, a complete double word sent to the flash memory must be finished in order to_
_clear the flag._
