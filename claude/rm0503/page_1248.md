...
*DP control and status register (DP_CTRLSTAT)*. The clock must be enabled before the
...
by setting a bit in the *DBGMCU configuration register (DBGMCU_CR)*. This also keeps the
...
information, refer to *Section 38.9: Microcontroller debug unit (DBGMCU)*.
