...
with two thresholds (1.65 V for PVM3 or 1.86 V for PVM4), refer to Section 4.2.3: Peripheral
...
The internal voltage reference is enabled by setting the ENVR bit in the Section 16.3.1:
...
- 2.5 V when the VRS bit is set and to 2.048 V when the VRS bit is cleared. The internal voltage reference can also provide the voltage to external components through VREF+ pin.
  Refer to the device datasheet and to Section 16: Voltage reference buffer (VREFBUF) for
  further information.
...
compared with the internal reference voltage (VREFINT, around 1.2 V), refer to Section 4.2.3:
