...
_Note:_
_When attempting to write in Fast programming mode while a read operation is ongoing, the_
...
_The 16 double words must be written successively. The high voltage is kept on the flash_
_memory for all the programming. Maximum time between two double words write requests_
_is the time programming (around 20 μs). If a second double word arrives after this time_
...
_If an error occurs, high voltage is stopped, and the next double word to programmed is not_
