...
Note:
For details on pin availability, refer to the pinout section in the corresponding device
datasheet.
...
1). Refer to Section 5.3: Low-power modes. It can also be used as a backup clock source
(auxiliary clock) if the HSE crystal oscillator fails. Refer to Section 5.2.10: Clock security
system (CSS).
...
RC oscillator. It can be trimmed using the HSITRIM[6:0] bits in the Internal clock sources
