...
_Note:_ _CC4S bits are writable only when the channel is OFF (CC4E = ‘0’ in TIMx_CCER)._
...
an internal trigger input is selected through TS bit (TIMx_SMCR register)
_Note:_ _CC3S bits are writable only when the channel is OFF (CC3E = ‘0’ in TIMx_CCER)._
