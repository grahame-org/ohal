...
_Note: SSRUF is not an error event as SSR counter is a free-running down-counter with automatic reload._
...
_Note: TSF is not set if TAMPTS = 1 and the tamper flag is read during the 3 ck_apre cycles following tamper event. Refer to Timestamp on tamper event for more details._
