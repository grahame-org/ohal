...
_Note: The data specified in Table 215 may slightly differ in the special case when the received frames contain some idle frames of exactly 10-bit times when M bits = 00 (11-bit times when M = 01 or 9- bit times when M = 10)._
...
_Note: When FIFO management is enabled and MME is already set, MME bit must not be cleared and then set again quickly (within two_ lpuart_ker_ck _cycles), otherwise mute mode might remain active._
