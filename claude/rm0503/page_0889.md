...
low power modes. Refer to _Section 29.3_ for details.
...
down-counter. The early interrupt is enabled by setting the EWIE bit of the _IWDG early_
_wake-up interrupt register (IWDG_EWCR)_ to 1.
...
In addition, the flag EWIF of the _IWDG status register (IWDG_SR)_ is set to 1.

The EWI interrupt is acknowledged by writing 1 to the EWIC bit in the _IWDG early wake-up_
_interrupt register (IWDG_EWCR)_.
