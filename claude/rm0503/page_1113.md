...
### 35.4.5 LPUART FIFOs and thresholds
...
_Note: The received data is stored in the RXFIFO together with the corresponding flags. However, only the data are read when reading the RDR._

_The status flags are available in the LPUART_ISR register._
...
configuration) on the TX pin. In this mode, the LPUART*TDR register consists of a buffer
(TDR) between the internal bus and the transmit shift register (see \_Section 35.4.1: LPUART block diagram*).
