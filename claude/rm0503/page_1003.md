...
1. The ADDR match event can wake up the device from Stop mode only if the I2C instance supports the wake-up from Stop
   mode feature. Refer to *Section 33.3: I2C implementation*.
...
peripheral (see *Section 11: Direct memory access controller (DMA)*) to the I2C_TXDR
...
configured through the DMA peripheral (refer to *Section 11: Direct memory access*
*controller (DMA)*) whenever the RXNE bit is set. Only the data are transferred with DMA.
