...
\_Note:_ \_If the Driver Enable feature is not supported, this bit is reserved and must be kept at reset value. Refer to **Section 34.4: USART implementation on page 1017**._
...
This bit can only be written when the USART is disabled (UE = 0).
\_Note:_ \_If the Driver Enable feature is not supported, this bit is reserved and must be kept at reset value.**Section 34.4: USART implementation on page 1017**._
...
This bit can only be written when the USART is disabled (UE = 0).
\_Note:_ \_The reception errors are: parity error, framing error or noise error._
...
USART*RDR register. When FIFO mode is enabled, the RXFIFO is bypassed and data are
written directly in USART_RDR register. Even when FIFO management is enabled, the
RXNE flag is to be used.
This bit can only be written when the USART is disabled (UE = 0).
\_Note:* _This control bit_enables_checking the communication flow w/o reading the data_
...
1: An interrupt is generated whenever CTSIF = 1 in the USART*ISR register
\_Note:* \_If the hardware flow control feature is not supported, this bit is reserved and must be kept at reset value. Refer to **Section 34.4: USART implementation**.\_
