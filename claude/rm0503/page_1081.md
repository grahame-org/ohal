...
_reset value. Refer to **Section 34.4: USART implementation on page 1017**._
...
This bit can only be written when the USART is disabled (UE = 0).
_Note:_ _If the Driver Enable feature is not supported, this bit is reserved and must be kept at_
_reset value.**Section 34.4: USART implementation on page 1017**._
...
This bit can only be written when the USART is disabled (UE = 0).
\_Note:* *This control bit *enables _checking the communication flow w/o reading the data_
...
\_Note:* _If the hardware flow control feature is not supported, this bit is reserved and must be_
