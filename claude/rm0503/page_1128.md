...
DMA mode can be enabled for reception by setting the DMAR bit in the LPUART*CR3
register. Data are loaded from the LPUART_RDR register to a SRAM area configured using
the DMA peripheral (refer to section \_direct memory access controller (DMA)*) whenever a
