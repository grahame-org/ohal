...
(when bit EN1 _=\_ 0 and bit CEN1 _=_ 0 in the DAC \_CR register). If EN1 _=_ 1 or CEN1 \_=_ 1 the
...
- DAC channel1 in sample & hold mode
  ...
  _Note: This register can be modified only when EN1 = 0._
...
in the latter case, the write can be done only when BWST1 of DAC*SR register is low, If
BWST1 *=\_ 1, the write operation is ignored.

\_Note: It represents the number of LSI clocks to perform a sample phase. Sampling time = (_TSAMPLE1[9:0] + 1) x \_LSI clock period._
