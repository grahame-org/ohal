...
- Device mode
  These bits contain information about the endpoint status, which are listed in _Table 232: Reception status encoding on page 1237_. These bits can be toggled by software to initialize
  ...
  the status based upon buffer availability condition (Refer to _Section 37.5.3: Double-buffered endpoints and usage in Device mode_).
