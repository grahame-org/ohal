...
0: The synchronous binary counter (SS[31:0] in RTC*SSR) is free-running.
...
\_Note:* _SSCLR must be kept to 0 when BCD or mixed mode is used (BIN = 00, 10 or 11)._
...
_Note:_ _In BCD mode (BIN=00) the overflow bits of the synchronous counter (bits 31:15) are_
_never compared. These bits can be different from 0 only after a shift operation._
