...
_Note: The TE bit must be set before writing the data to be transmitted to the LPUART_TDR._

_The TE bit must not be reset during data transmission. Resetting the TE bit during the transmission corrupts the data on the TX pin as the baud rate counters is frozen. The current data being transmitted are lost._
...
- **_1 stop bit_\*\***:\*\* This is the default value of the number of stop bits.
- **_2 stop bits_\*\***:\*\* This is supported by normal LPUART, single-wire, and modem modes.
...
5. Select DMA enable (DMAT) in LPUART*CR3 if multibuffer communication is to take
   place. Configure the DMA register as explained in \_Section 35.4.13: Continuous communication using DMA and LPUART*.
