...
the way explained in _Section 22.4.6: AES ciphertext stealing and data padding_.

For more information on data swapping, refer to _Section 22.4.13: AES data registers and data swapping_.

#### ECB/CBC encryption sequence

The sequence of events to perform an ECB/CBC encryption (more detail in \_Section 22.4.4\*):
...
6. Write the AES*DINR register four times to input the plaintext (MSB first), as shown in
   \_Figure 107*.
7. Wait until the CCF flag is set in the AES_SR register.
8. Read the AES*DOUTR register four times to get the ciphertext (MSB first) as shown in
   \_Figure 107*. Then clear the CCF flag by setting the CCF bit of the AES_CR register.
9. Repeat steps*6*-_7_-_8_ to process all the blocks with the same encryption key.
...
1. Follow the steps described in _Section 22.4.5: AES decryption round key preparation_, in
