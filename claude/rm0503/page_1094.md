...
\_Note:_ \_If the USART does not support LIN mode, this bit is reserved and kept at reset value. Refer to **Section 34.4: USART implementation**._
...
\_Note:_ \_The TXFNF is kept reset during the flush request until TXFIFO is empty. After sending the flush request (by setting TXFRQ bit), the flag TXFNF must be checked prior to writing in TXFIFO (TXFNF and TXFE is set at the same time). This bit is used during single buffer transmission._
...
\_Note:_ \_If the TE bit is reset and no transmission is ongoing, the TC bit is immediately set._
