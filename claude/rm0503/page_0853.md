...
The CC3IF flag is set by hardware to inform application that LPTIM*CNT register value matches the
...
The CC2IF flag is set by hardware to inform application that LPTIM*CNT register value matches the
...
LPTIM*ICR register.
\_Note:* _If the LPTIM does not support encoder mode feature, this bit is reserved. Refer to **Section 27.3**._
...
changed from down to up. UP flag can be cleared by writing 1 to the UPCF bit in the LPTIM*ICR
register.
\_Note:* _If the LPTIM does not support encoder mode feature, this bit is reserved. Refer to **Section 27.3**._
