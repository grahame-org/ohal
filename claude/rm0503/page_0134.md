...
111: External input analog voltage PVD*IN (compared internally to VREFINT)
\_Note:* _These bits are write-protected when the bit PVDL (PVD Lock) is set in the SYSCFG_CBR register. These bits are reset only by a system reset._
...
_Note:_ _This bit is write-protected when the bit PVDL (PVD Lock) is set in the SYSCFG_CBR register. This bit is reset only by a system reset._
