...
_Note:_ _It is recommended to set the UESM bit just before entering low-power mode, and clear_
_it when exiting low-power mode._
...
1: LPUART enabled
\_Note:* _To enter low-power mode without generating errors on the line, the TE bit must be reset_
_before and the software must wait for the TC bit in the LPUART_ISR to be set before_
_resetting the UE bit._
