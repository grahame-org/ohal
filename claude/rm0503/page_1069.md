...
1: USART interrupt generated when the EOBF flag is set in the USART*ISR register
...
This bit is set and cleared by software.
0: Interrupt inhibited
1: USART interrupt generated when the RTOF bit is set in the USART*ISR register.
\_Note:* \_If the USART does not support the Receiver timeout feature, this bit is reserved and must be kept at reset value.**Section 34.4: USART implementation**.\_
...
\_Note:_ \_If the Driver Enable feature is not supported, this bit is reserved and must be kept at reset value. Refer to **Section 34.4: USART implementation**._
...
If the USART*TDR register is written during the DEDT time, the new data is transmitted only
when the DEDT and DEAT times have both elapsed.
This bitfield can only be written when the USART is disabled (UE = 0).
\_Note:* \_If the Driver Enable feature is not supported, this bit is reserved and must be kept at reset value. Refer to **Section 34.4: USART implementation**.\_
...
\_Note:_ \_In LIN, IrDA and smartcard modes, this bit must be kept cleared._
