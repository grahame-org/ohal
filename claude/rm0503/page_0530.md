...
known length Len(A) that can be a non-multiple of 16 bytes (see _Figure 116_). The
...
- **16-byte blocks (B)** associated to the plaintext message P, which is both authenticated
  ...
  multiple of 16 bytes (see _Figure 116_).
...
_Note: CCM chaining mode can also be used with associated data only (that is, no payload)._
