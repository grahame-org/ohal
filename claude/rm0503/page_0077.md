...
_Note: If LOCK is set by software, OPTLOCK is automatically set as well._
...
_Note: Any modification of the value of one option is automatically performed by erasing user option byte pages first, and then programming all the option bytes with the values contained in the flash memory option registers._

_The complementary values are automatically computed and written into the complemented option bytes upon setting the OPTSTRT bit._
...
- when OBL_LAUNCH bit of the _FLASH control register (FLASH \_CR)_ is set
