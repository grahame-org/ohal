...
_Note: A detected configurable event interrupt pending request can be cleared by the CPU. The system cannot enter low-power modes as long as an interrupt pending request is active._

### 12.3.2 EXTI direct event input wake-up

_Figure 28_ is a detailed representation of the logic associated with direct event inputs waking
up the system.
...
selection of GPIO port as EXTI mux output is controlled through the _EXTI external interrupt selection register x (EXTI_EXTICRx)_ register.
