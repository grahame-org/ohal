...
_Note:_
_The RXNE/RXFNE interrupt must be enabled even when using the USART in DMA mode to_
...
_Note:_
_As in the smartcard protocol definition, the BWT/CWT values must be defined from the_
_beginning (start bit) of the last character. The RTO register must be programmed to BWT_-
