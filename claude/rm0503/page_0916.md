...
After an initialization (refer to Calendar initialization and configuration on page 914): the
software must wait until RSF is set before reading the RTC_SSR, RTC_TR and RTC_DR
registers.

After synchronization (refer to Section 31.3.12: RTC synchronization): the software must
