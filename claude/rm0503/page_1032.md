...
- select the three sample majority vote method (ONEBIT = 0) when operating in a
  ...
  Table 203) because this indicates that a glitch occurred during the sampling.
- select the single sample method (ONEBIT = 1) when the line is noise-free to
  increase the receiver tolerance to clock deviations (see Section 34.5.9: Tolerance
