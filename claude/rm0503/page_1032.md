...
- select the single sample method (ONEBIT = 1) when the line is noise-free to
  increase the receiver tolerance to clock deviations (see _Section 34.5.9: Tolerance_
  _of the USART receiver to clock deviation_). In this case the NE bit is never set.
...
_Note:_
_Noise error is not supported in SPI and IrDA modes._
