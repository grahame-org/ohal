...
- PC13, PC14 and PC15, which can be configured by RTC or LSE (refer to _Section 31.3: RTC functional description on page 905_)
- PA0/RTC_TAMP2 and PE6/RTC_TAMP3 when they are configured by the RTC as
  tamper pins

_Note: Due to the fact that the analog switch can transfer only a limited amount of current (3 mA), the use of GPIO PC13 to PC15 in output mode is restricted: the speed has to be limited to_
_2 MHz with a maximum load of 30 pF and these I/Os must not be used as a current source\_
_(e.g. to drive a LED).\*
