...
_Note:_
_The payload phase can be skipped if there is no payload data, that is, Len(C) = 0 (see_
_GMAC mode)._
...
_Note:_
_In the final phase, data is written to AES_DINR normally (no swapping), while swapping is_
_applied to tag data read from AES_DOUTR._

_When transiting from the header or the payload phase to the final phase, the AES peripheral_
_must not be disabled, otherwise the result is wrong._
