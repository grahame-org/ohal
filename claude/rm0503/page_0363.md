...
*Note: The sensor has a startup time after waking from power down mode before it can output V \_*SENSE \*_at the correct level. The ADC also has a startup time after power-on, so to minimize the delay, the ADEN and TSEN bits should be set at the same time._
...
reference and VREF+ can be replaced by the expression provided in *Calculating the actual V \_*REF+\*_voltage using the internal reference voltage_, resulting in the following formula:
