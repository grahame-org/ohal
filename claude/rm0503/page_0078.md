...
\_Note: If the read protection is programmed through software, do not set the OBL*LAUNCH bit (*\_FLASH control register (FLASH*CR) register) to reload the option byte. Instead, perform a POR. This can be done by entering Standby or Shutdown mode then waking up the device*
