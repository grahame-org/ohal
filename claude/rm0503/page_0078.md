...
they reflect the options states of the system. See Section : Modifying user options for more
details.
...
Note:
If the read protection is programmed through software, do not set the OBL_LAUNCH bit
(FLASH control register (FLASH_CR) register) to reload the option byte. Instead, perform a
POR. This can be done by entering Standby or Shutdown mode then waking up the device
...
pair of values shown in Table 14.
