...
_Note:_ _The software is allowed to set ADCAL only when the ADC is disabled (ADCAL_ = _0,_
...
_Note:_ _The software is allowed to program this bit field only when the ADC is disabled (ADCAL_ = _0,_
...
_Note:_ _Setting ADSTP to ‘1’ is only effective when ADSTART_ = _1 and ADDIS_ = _0 (ADC is enabled and_
...
- In all other cases: after the execution of the ADSTP command, at the same time as the ADSTP bit is
  ...
  _Note:_ _The software is allowed to set ADSTART only when ADEN_ = _1 and ADDIS_ = _0 (ADC is enabled_
