...
The RECALPF status flag is automatically set to 1 when software writes to the RTC*CALR
register, indicating that the RTC_CALR register is blocked. When the new calibration settings
are taken into account, this bit returns to 0. Refer to \_Re-calibration on-the-fly*.
