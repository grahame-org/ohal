...
OC1REFC outputs OC1REF when the counter is counting up, OC2REF when it is counting
down.
\_Note:* \_In PWM mode, the OCREF level changes only when the result of the comparison changes or when the output compare mode switches from “frozen” mode to “PWM” mode. Note:_ \_The OC1M[3] bit is not contiguous, located in bit 16._
