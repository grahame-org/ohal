...
_Note:_
_If the CSS is enabled and the HSE clock fails, the CSSI occurs and an NMI is automatically_
_generated. The NMI is executed infinitely unless the CSS interrupt pending bit is cleared. It_
_is therefore necessary that the NMI ISR clears the CSSI by setting the CSSC bit in the \*\*Clock_
_interrupt clear register (RCC_CICR)\*\*._
...
A clock security system on LSE can be activated by setting the LSECSSON bit in _RTC_
_domain control register (RCC_BDCR)_. This bit can be cleared only by a hardware reset or
...
_Note:_
_If the LSECSS is enabled and the LSE clock fails, the LSECSSI occurs and an NMI is_
...
_setting the LSECSSC bit in the **Clock interrupt clear register (RCC_CICR)**._
