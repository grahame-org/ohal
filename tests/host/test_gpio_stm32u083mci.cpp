...
// ---------------------------------------------------------------------------
// Host-side tests for the STM32U083MCI (81-pin UFBGA) GPIO model.
//
// The STM32U083MCI exposes GPIOA, GPIOB, GPIOC, GPIOD, GPIOE, and GPIOF.
// GPIOD is partially bonded (PD7, PD14, PD15 absent).
// GPIOE is partially bonded (PE3, PE7–PE9; all other PE pins absent).
// GPIOF is partially bonded (PF0–PF3 only).
// These tests verify:
//   1. The capability traits report the correct values for the MCI package.
...
class GpioStm32u083MciTest : public ::testing::Test {
protected:
  void SetUp() override {
    mock_moder = 0U;
    mock_otyper = 0U;
...
TEST_F(GpioStm32u083MciTest, Set_WritesBitToBsrr) {
  MockPin5::set();
  EXPECT_EQ(mock_bsrr, 1U << 5U);
}

TEST_F(GpioStm32u083MciTest, Clear_WritesBsrrResetBit) {
  MockPin5::clear();
  EXPECT_EQ(mock_bsrr, 1U << 21U);
}

TEST_F(GpioStm32u083MciTest, SetMode_Output_WritesModerBits) {
  MockPin5::set_mode(ohal::gpio::PinMode::Output);
  EXPECT_EQ(mock_moder & (0b11U << 10U), 0b01U << 10U);
}

TEST_F(GpioStm32u083MciTest, SetMode_Input_WritesModerBits) {
  mock_moder = 0b11U << 10U;
  MockPin5::set_mode(ohal::gpio::PinMode::Input);
  EXPECT_EQ(mock_moder & (0b11U << 10U), 0U);
}

TEST_F(GpioStm32u083MciTest, ReadInput_ReturnsHigh_WhenIdrBitSet) {
  mock_idr = 1U << 5U;
  EXPECT_EQ(MockPin5::read_input(), ohal::gpio::Level::High);
}

TEST_F(GpioStm32u083MciTest, PortSet_WritesMaskToBsrrLow16Bits) {
  MockPort::set(0x00FFU);
  EXPECT_EQ(mock_bsrr, 0x00FFU);
}

TEST_F(GpioStm32u083MciTest, PortClear_WritesMaskToBsrrHigh16Bits) {
  MockPort::clear(0x00FFU);
  EXPECT_EQ(mock_bsrr, 0x00FF0000U);
}

TEST_F(GpioStm32u083MciTest, PortWrite_CombinesSetAndClearMasksInSingleBsrrWrite) {
...
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortA, 16>::value,
              "PortA pin 16 (out of range) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_output_speed<ohal::gpio::PortA, 16>::value,
              "PortA pin 16 (out of range) must not report supports_output_speed on MCI");
static_assert(!ohal::gpio::capabilities::supports_pull<ohal::gpio::PortA, 16>::value,
              "PortA pin 16 (out of range) must not report supports_pull on MCI");
static_assert(!ohal::gpio::capabilities::supports_alternate_function<ohal::gpio::PortA, 16>::value,
              "PortA pin 16 (out of range) must not report supports_alternate_function on MCI");

// Non-bonded PortD pins: PD7, PD14, PD15.
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortD, 7>::value,
              "PortD pin 7 (not bonded) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortD, 14>::value,
              "PortD pin 14 (not bonded) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortD, 15>::value,
              "PortD pin 15 (not bonded) must not report supports_output_type on MCI");

// Non-bonded PortE pins on the MCI: PE0–PE2, PE4–PE6, PE10–PE15.
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 0>::value,
              "PortE pin 0 (not bonded) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 2>::value,
              "PortE pin 2 (not bonded) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 4>::value,
              "PortE pin 4 (not bonded) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 6>::value,
              "PortE pin 6 (not bonded) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 10>::value,
              "PortE pin 10 (not bonded) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 15>::value,
              "PortE pin 15 (not bonded) must not report supports_output_type on MCI");

// Non-bonded PortF pins: PF4–PF15.
static_assert(!ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortF, 4>::value,
              "PortF pin 4 (not bonded) must not report supports_output_type on MCI");
static_assert(!ohal::gpio::capabilities::supports_alternate_function<ohal::gpio::PortF, 15>::value,
              "PortF pin 15 (not bonded) must not report supports_alternate_function on MCI");
...
class GpioStm32u083MciCapabilityTest : public ::testing::TestWithParam<CapCase> {};

// clang-format off
INSTANTIATE_TEST_SUITE_P(
    BondedPortCapabilities, GpioStm32u083MciCapabilityTest,
    ::testing::Values(
        ...
        CapCase{ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 3>::value,
...
TEST_P(GpioStm32u083MciCapabilityTest, CapabilityIsTrue) { EXPECT_TRUE(GetParam().value); }
...
class GpioStm32u083MciInvalidCapTest : public ::testing::TestWithParam<InvalidCapCase> {};

// clang-format off
INSTANTIATE_TEST_SUITE_P(
    UnbondedAndOutOfRangeCapabilities, GpioStm32u083MciInvalidCapTest,
    ::testing::Values(
        ...
        // Non-bonded PortE pins (PE0–PE2, PE4–PE6, PE10–PE15)
        InvalidCapCase{
            ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 0>::value,
            "PortE_Pin0_OutputType"},
        InvalidCapCase{
            ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 2>::value,
            "PortE_Pin2_OutputType"},
        InvalidCapCase{
            ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 4>::value,
            "PortE_Pin4_OutputType"},
        InvalidCapCase{
        ...
        InvalidCapCase{
            ohal::gpio::capabilities::supports_output_type<ohal::gpio::PortE, 15>::value,
            "PortE_Pin15_OutputType"},
...
TEST_P(GpioStm32u083MciInvalidCapTest, CapabilityIsFalse) { EXPECT_FALSE(GetParam().value); }

// ---------------------------------------------------------------------------
// Port-wiring tests: verify that Pin<PortX, N> resolves to the correct MMIO
// base address for the STM32U083MCI package.
// ---------------------------------------------------------------------------
...
class GpioStm32u083MciWiringTest : public ::testing::TestWithParam<WiringCase> {};

// clang-format off
INSTANTIATE_TEST_SUITE_P(
    PortBsrrAddresses, GpioStm32u083MciWiringTest,
    ::testing::Values(
...
TEST_P(GpioStm32u083MciWiringTest, BsrrAddressMatchesHardwareBase) {
