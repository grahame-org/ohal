...
// ---------------------------------------------------------------------------
// Host-side tests for the STM32U083HCY (42-ball WLCSP) GPIO model.
//
// The STM32U083HCY shares the same GPIO register map and bonded-out port set
// (GPIOA, GPIOB, GPIOC, GPIOF) as the STM32U083KCU.  These tests verify:
//   1. The capability traits report the correct values for the HCY package.
...
class GpioStm32u083HcyTest : public ::testing::Test {
protected:
  void SetUp() override {
    mock_moder = 0U;
    mock_otyper = 0U;
...
TEST_F(GpioStm32u083HcyTest, Set_WritesBitToBsrr) {
  MockPin5::set();
  EXPECT_EQ(mock_bsrr, 1U << 5U);
}

TEST_F(GpioStm32u083HcyTest, Clear_WritesBsrrResetBit) {
  MockPin5::clear();
  EXPECT_EQ(mock_bsrr, 1U << 21U);
}

TEST_F(GpioStm32u083HcyTest, SetMode_Output_WritesModerBits) {
  MockPin5::set_mode(ohal::gpio::PinMode::Output);
  EXPECT_EQ(mock_moder & (0b11U << 10U), 0b01U << 10U);
}

TEST_F(GpioStm32u083HcyTest, SetMode_Input_WritesModerBits) {
  mock_moder = 0b11U << 10U;
  MockPin5::set_mode(ohal::gpio::PinMode::Input);
  EXPECT_EQ(mock_moder & (0b11U << 10U), 0U);
}

TEST_F(GpioStm32u083HcyTest, ReadInput_ReturnsHigh_WhenIdrBitSet) {
  mock_idr = 1U << 5U;
  EXPECT_EQ(MockPin5::read_input(), ohal::gpio::Level::High);
}

TEST_F(GpioStm32u083HcyTest, PortSet_WritesMaskToBsrrLow16Bits) {
  MockPort::set(0x00FFU);
  EXPECT_EQ(mock_bsrr, 0x00FFU);
}

TEST_F(GpioStm32u083HcyTest, PortClear_WritesMaskToBsrrHigh16Bits) {
  MockPort::clear(0x00FFU);
  EXPECT_EQ(mock_bsrr, 0x00FF0000U);
}

TEST_F(GpioStm32u083HcyTest, PortWrite_CombinesSetAndClearMasksInSingleBsrrWrite) {
...
class GpioStm32u083HcyCapabilityTest : public ::testing::TestWithParam<CapCase> {};

// clang-format off
INSTANTIATE_TEST_SUITE_P(
    BondedPortCapabilities, GpioStm32u083HcyCapabilityTest,
    ::testing::Values(
...
TEST_P(GpioStm32u083HcyCapabilityTest, CapabilityIsTrue) { EXPECT_TRUE(GetParam().value); }
...
class GpioStm32u083HcyInvalidCapTest : public ::testing::TestWithParam<InvalidCapCase> {};

// clang-format off
INSTANTIATE_TEST_SUITE_P(
    UnbondedAndOutOfRangeCapabilities, GpioStm32u083HcyInvalidCapTest,
    ::testing::Values(
        ...
        // PortD — not bonded out on the HCY 42-ball WLCSP package
        InvalidCapCase{
        ...
        // PortE — not bonded out on the HCY 42-ball WLCSP package
        InvalidCapCase{
...
TEST_P(GpioStm32u083HcyInvalidCapTest, CapabilityIsFalse) { EXPECT_FALSE(GetParam().value); }

// ---------------------------------------------------------------------------
// Port-wiring tests: verify that Pin<PortX, N> resolves to the correct MMIO
// base address for the STM32U083HCY package.
// ---------------------------------------------------------------------------
...
class GpioStm32u083HcyWiringTest : public ::testing::TestWithParam<WiringCase> {};

// clang-format off
INSTANTIATE_TEST_SUITE_P(
    PortBsrrAddresses, GpioStm32u083HcyWiringTest,
    ::testing::Values(
...
TEST_P(GpioStm32u083HcyWiringTest, BsrrAddressMatchesHardwareBase) {
