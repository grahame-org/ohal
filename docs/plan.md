...
│       ├── family.hpp
│       └── models/
│           └── pic18f4550/              ← non-ARM concrete implementation
│               ├── gpio.hpp
│               └── capabilities.hpp
├── tests/
│   ├── host/
...
Each step is documented in its own file. Steps must be completed in order as later steps depend on
earlier ones.

| Step | File | Inputs Required |
|------|------|-----------------|
| 1 | [Build Infrastructure](steps/step-01-build-infrastructure.md) | None |
| 2 | [Core Register Abstraction](steps/step-02-register-abstraction.md) | None |
| 3 | [BitField and Access Control](steps/step-03-bitfield-access-control.md) | None |
| 4 | [MCU Family/Model Selection](steps/step-04-mcu-selection.md) | STM32U083 register map |
| 5 | [GPIO Peripheral Interface](steps/step-05-gpio-interface.md) | None |
| 6 | [STM32U0 GPIO Implementation](steps/step-06-stm32u0-gpio.md) | STM32U083 GPIO register map |
| 7 | [Timer and UART Peripherals](steps/step-07-timer-uart.md) | STM32U083 TIM2 and USART2 register maps |
| 8 | [PIC18F4550 GPIO (non-ARM)](steps/step-08-pic18f4550-gpio.md) | PIC18F4550 PORT/LAT/TRIS register map |
| 9 | [Host and Target Unit Testing](steps/step-09-unit-testing.md) | None |
| 10 | [CI / Continuous Integration](steps/step-10-ci.md) | ARM and XC8 toolchains |
...
|---|---|
| Clock / RCC | GPIO peripheral clocks must be enabled before registers can be accessed. Should OHAL manage clock enabling, or leave this to the BSP? If managed: requires RCC register maps per model. |
...
| TI MSP-M0 | MSP-M0 GPIO uses 32-bit registers with a different layout from STM32. Register maps and capability traits must be gathered from the MSPM0G3507 Technical Reference Manual before implementation. |
